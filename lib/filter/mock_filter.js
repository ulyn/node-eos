...
//            console.error("mock config file is no found ,please check the path of %s is exists",filename);
            throw new Error("mock config file is no found ,please check the path of "+filename+" is exists");
        }
    });
}
...
function readConfig(filename){
    try{
        ...
        throw new Error("mock config parse json exception."+"name="+e.name+",message="+e.message);
    }
}
