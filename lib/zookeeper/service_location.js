...
util.inherits(ServiceLocation, EventEmitter);
ServiceLocation.prototype.connect = function connect(callback) {
    var self = this;
    if(self.zookeeper){
        ...
        self.once("loadService",function(){
            if(callback){
                callback(self);
            }
        })
...
//            console.log('Got path of %s watcher event: %s', path,event);
        ...
        function (error, children, stat) {
            ...
            serviceLocation.emit("loadService");
        });
}
